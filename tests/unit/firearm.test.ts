...
describe('射击与装填', () => {
  ...
  it('战术换弹也在 1.55 秒完成，重复换弹不重置进度', () => {
    const gun = new Firearm(); gun.fire(); gun.reload();
    expect(gun.reloadRemaining).toBe(1.55); expect(gun.reloadEmpty).toBe(false);
    ...
    gun.update(1.15); expect(gun.ammo).toBe(30); expect(gun.reloading).toBe(false);
  });
