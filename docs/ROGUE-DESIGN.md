...
本文件为设计稿，未实现新玩法。用户明确提出的规则作为设计基础；波次配置、升级幅度、出场顺序及交互细节为首轮试玩建议，可在开发前调整。现有 README 继续介绍已实现的游戏。
...
技术验收继续使用单浏览器、单 worker；必要逻辑测试覆盖波次计数、升级公式、排序和落点约束。初版设计确认后再开始实现，不在本轮创建新模型或修改成品包。
