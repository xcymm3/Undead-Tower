...
成品：`release/Undead Tower 0.5.0.exe`。面向 Windows 10/11 x64，已同步 2026-09-04 当前游戏源码。

本版包含六款无手部悬浮枪械、数字键/滚轮切换、独立弹量、协调的开火/换弹动画及轻量平衡；僵尸自动绕过场景实体障碍，左右刷怪位置内收；取消可见防线，失败时先播放两秒突破者特写，再自动显示排行榜。所有正式模式僵尸移速固定为 **1.4 米/秒**，仅刷新率随时间增长，最高每秒 10 只。保留固定困难与最快镜头跟随、护甲脱落与受击声、个人纪录提示、音量调节、死亡低吼与低音量 BGM。

旧的 0.3.0、0.4.0 EXE 保留作历史版本，请运行 **Undead Tower 0.5.0.exe**（内部版本号 0.5.0）。后续默认只修改源码和网页版，用户明确要求后才重新打包。
...
鼠标瞄准，左键开火，R 换弹，数字键 1–6 或鼠标滚轮切换枪械，Esc 暂停，M 静音。右上角按钮切换全屏，也支持 F11；窗口关闭按钮或 Alt+F4 退出。
...
成品测试在 `test-results/portable-时间戳/` 中创建独立副本和空白数据，单窗口验证离线加载、六款枪械开火与换弹、数字键与滚轮切换、暂停、全屏、固定 1.4 米/秒移速、护甲刷新、自然失败与两秒突破者特写、保存和搬迁后重启读取成绩。测试不会把成绩写入交付 EXE 旁。调试端口仅由测试命令临时指定；普通双击启动不打开调试端口。截图和结果 JSON 保存在本次测试目录。
