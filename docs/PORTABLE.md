...
成品：`release/Undead-Tower-0.3.0-portable-x64.exe`。面向 Windows 10/11 x64，保留 2026-09-03 的游戏构建快照。2026-09-04 的圆弧突破、轻微避让、固定困难与最快镜头跟随、防线/失败提示、护甲脱落与声音、个人纪录提示、音量调节均尚未打入 EXE；默认只修改源码和网页版，用户明确要求后才重新打包。

后续新增的强化护具受击声、死亡低吼、低音量循环 BGM 也仅在源码/网页版中，当前 EXE 不包含这些声音。

0.775 秒快速换弹、减少遮挡的持枪姿态、左手与独立弹匣换弹动作同样尚未打入当前 EXE。
...
EXE 旁会自动创建 `Undead Tower Data` 文件夹，保存便携版排行榜和运行缓存。更换目录或电脑时，将 **EXE 和这个数据文件夹一起拷贝**。只拷贝 EXE 也能启动，但目标位置将创建新纪录。它与 Chrome/Edge 网页版的排行榜独立，各难度仍分榜；不会自动读取或更改网页成绩。
