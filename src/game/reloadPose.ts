...
export function reloadPose(progress: number | null, empty = false) {
  ...
  return {
    stage: progress === null || p >= 1 ? 'idle' : p < 0.14 ? 'tilt' : p < 0.36 ? 'remove' : p < 0.71 ? 'insert' : p < 0.80 ? 'seat' : p < 0.91 && empty ? 'bolt' : 'settle',
    ...
    bolt: empty ? smooth((p - 0.83) / 0.04) * (1 - smooth((p - 0.9) / 0.035)) : 0,
  };
}
